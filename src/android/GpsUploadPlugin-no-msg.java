    ...
    Handler mHandler = new Handler() {
        public void handleMessage(android.os.Message msg)
        {
            if (mLocationClient != null) {
                ...
                GpsData gps = (GpsData)data.get("gpsData");
                Log.e("myTag", " requestLocation  result   " + result);
                Log.e("myTag-Message--", " GpsData  getPlateNo   " + data.toString());
                if (gps != null) {
                    Log.e("myTag-Message--", " GpsData  getPlateNo   " + gps.getPlateNo());
        ...
        if (mLocationClient == null) {
            ...
            mLocationClient.registerLocationListener(new BDLocationListener()
            {
                ...
                public void onReceiveLocation(BDLocation loc)
                {
                    ...
                    Log.e("myTag", " onReceiveLocation  result   " + loc.getLongitude() + " , " + loc.getLatitude() + " , 方向" + loc.getDirection() + ",高度" + loc.getAltitude() + ",速度" + loc.getSpeed());
                    try {

                        GpsData bean = new GpsData();
                        int altitude = (int)
                                loc.getAltitude(); bean.setAltitude(altitude);
                        bean.setLongitude(loc.getLongitude() * 1000000);
                        bean.setLatitude(loc.getLatitude() * 1000000); int
                                mDirection = (int) loc.getDirection();
                        bean.setDirection(mDirection); int speed = (int)
                                loc.getSpeed(); bean.setGpsSpeed(speed); //
                            ...
                            if (fatUrl != null && !"".equals(fatUrl)) {
                                ...
                                new Thread(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        uploadController.putOneGpsBean(newBean, activity.getApplicationContext(), host, port);
                                        PluginResult mPlugin2 = new PluginResult(PluginResult.Status.OK,
                                                gsonFat.toJson(newBean));
                                        mPlugin2.setKeepCallback(true);
                                        callbackContext.sendPluginResult(mPlugin2);
                                    }
                                }).start();
                            } else {
                                uploadController.putOneGpsBean(bean, activity.getApplicationContext(), host, port);
